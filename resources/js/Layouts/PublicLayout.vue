      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex items-center justify-between h-16">
          ...
          <Link href="/" class="flex items-center gap-3 group">
            ...
            <div class="w-10 h-10 rounded-lg bg-white/20 items-center justify-center flex-shrink-0 hidden">
            ...
            <div class="leading-tight">
              <p class="text-sm font-bold text-white group-hover:text-white/80 transition-colors">CSC RO VIII</p>
    ...
    <footer class="bg-primary-dark">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
          ...
          <div>
            <div class="flex items-center gap-2 mb-3">
              ...
              <img src="/images/csc-logo.png" alt="CSC Logo" class="h-7 w-7 object-contain"
                onerror="this.style.display='none';this.nextElementSibling.style.display='flex'" />
              <div class="w-7 h-7 rounded bg-white/20 items-center justify-center hidden">
                <svg class="w-4 h-4 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                  <path stroke-linecap="round" stroke-linejoin="round" d="M9 12l2 2 4-4M7.835 4.697a3.42 3.42 0 001.946-.806 3.42 3.42 0 014.438 0 3.42 3.42 0 001.946.806 3.42 3.42 0 013.138 3.138 3.42 3.42 0 00.806 1.946 3.42 3.42 0 010 4.438 3.42 3.42 0 00-.806 1.946 3.42 3.42 0 01-3.138 3.138 3.42 3.42 0 00-1.946.806 3.42 3.42 0 01-4.438 0 3.42 3.42 0 00-1.946-.806 3.42 3.42 0 01-3.138-3.138 3.42 3.42 0 00-.806-1.946 3.42 3.42 0 010-4.438 3.42 3.42 0 00.806-1.946 3.42 3.42 0 013.138-3.138z"/>
                </svg>
              </div>
              <span class="text-sm font-bold text-white">CSC RO VIII</span>
            </div>
          ...
          <!-- Col 2: Quick links -->
          <div>
            <h3 class="text-xs font-semibold text-white uppercase tracking-wider mb-3">Quick Links</h3>
            <ul class="space-y-2">
              <li><Link href="/" class="text-sm text-white/60 hover:text-white transition-colors">Open Vacancies</Link></li>
        ...
        <div class="mt-8 pt-6 border-t border-white/10 flex flex-col sm:flex-row justify-between items-center gap-2">
          ...
          <div class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/5 border border-white/10">
            <svg class="w-3.5 h-3.5 text-white/40" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
              <path stroke-linecap="round" stroke-linejoin="round" d="M15 19.128a9.38 9.38 0 002.625.372 9.337 9.337 0 004.121-.952 4.125 4.125 0 00-7.533-2.493M15 19.128v-.003c0-1.113-.285-2.16-.786-3.07M15 19.128v.106A12.318 12.318 0 018.624 21c-2.331 0-4.512-.645-6.374-1.766l-.001-.109a6.375 6.375 0 0111.964-3.07M12 6.375a3.375 3.375 0 11-6.75 0 3.375 3.375 0 016.75 0zm8.25 2.25a2.625 2.625 0 11-5.25 0 2.625 2.625 0 015.25 0z" />
            </svg>
            <span class="text-[11px] text-white/50 font-medium tracking-wide">TOTAL VISITORS</span>
...
onMounted(async () => {
  ...
  try {
    if (alreadyCounted) {
      const { data } = await api.get('/visitor-count')
      visitorCount.value = data.total
    } else {
      const { data } = await api.post('/visitor-count/increment', { page: 'home' })
      sessionStorage.setItem('visitor_counted', '1')
      visitorCount.value = data.total
    }
  } catch {
    const { data } = await api.get('/visitor-count').catch(() => ({ data: { total: 0 } }))
    visitorCount.value = data.total
  }
})
</script>